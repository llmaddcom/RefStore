...
class MinioFileService:
    ...
    支持可选的逻辑桶名到物理桶名映射（默认关闭，遵循 AWS S3 最佳实践）。
        ...
        config: 配置字典，格式如下：
            {
                ...
                "enable_bucket_mapping": False,   # 默认关闭，开启后使用 bucket_map
                "bucket_map": {                    # 仅在 enable_bucket_mapping=True 时生效
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.secure = secure
        self.enable_bucket_mapping = self.config.get("enable_bucket_mapping", False)
        self.bucket_map = self.config.get("bucket_map", {})
        self.default_bucket = self.config.get("default_bucket", "default")
        self.presigned_expiry = self.config.get("presigned_expiry", 3600)
        self.public_url = self.config.get("public_url")

        # 构建反向映射表：物理桶名 -> 逻辑桶名
        self.reverse_bucket_map: Dict[str, str] = {}
        if self.enable_bucket_mapping and self.bucket_map:
            self.reverse_bucket_map = {v: k for k, v in self.bucket_map.items()}
    ...
    def get_logical_bucket(self, physical_bucket: str) -> str:
        ...
        return self.reverse_bucket_map.get(physical_bucket, physical_bucket)
    ...
    def _build_s3_uri(
        ...
        映射关闭时：始终返回物理桶 URI。
        映射开启时：use_logical_uri=True 返回逻辑桶 URI，否则返回物理桶 URI。
        """
        if not self.enable_bucket_mapping:
            return URIUtils.encode_uri(physical_bucket, object_name)
        if use_logical_uri:
            return URIUtils.encode_uri(logic_bucket, object_name)
        return URIUtils.encode_uri(physical_bucket, object_name)

    def _resolve_uri_to_physical(self, s3_uri: str) -> tuple:
        """
        解析 S3 URI，返回 (physical_bucket, object_name)。
        兼容逻辑桶 URI 和物理桶 URI 两种输入。
        """
        bucket, object_name = URIUtils.decode_uri(s3_uri)
        ...
        if bucket in self.bucket_map:
            return self.bucket_map[bucket], object_name

        # 再尝试作为物理桶名（已经是物理桶）
        if bucket in self.reverse_bucket_map:
            return bucket, object_name
    ...
    def upload_file(
        ...
        :param logic_bucket: 逻辑桶名（映射开启时会映射到物理桶，关闭时即为物理桶名）
        ...
        try:
            ...
            return self._build_s3_uri(logic_bucket, physical_bucket, object_name, use_logical_uri)
        except Exception as e:
    ...
    def download_file(self, s3_uri: str) -> Optional[bytes]:
        """下载文件到内存"""
        try:
            physical_bucket, object_name = self._resolve_uri_to_physical(s3_uri)
    ...
    def get_file_info(self, s3_uri: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            physical_bucket, object_name = self._resolve_uri_to_physical(s3_uri)
            logic_bucket = self.get_logical_bucket(physical_bucket)
            ...
            return [
                {
                    ...
                    "s3_uri": self._build_s3_uri(
                        logic_bucket, physical_bucket, obj.object_name, use_logical_uri
                    ),
                }
                for obj in objects
            ]
